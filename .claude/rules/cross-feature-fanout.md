...
- El `ApplicationContainer` raíz arma `group_stats_providers =
  ...
  **antes** de construir `activity` (`activity =
  providers.Container(ActivityContainer, pool=db_pool,
  ...
  `points` y `polls` ya estaban declarados antes de `activity`, así que agregar
  un provider nuevo a la lista no obligó a reordenar nada — solo hace falta
  reordenar si la feature que aporta se declara DESPUÉS del agregador.
